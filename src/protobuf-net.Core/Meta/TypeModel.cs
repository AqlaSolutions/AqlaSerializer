...
#if !FEAT_IKVM
        private object DeserializeRootAny(ProtoReader reader, Type type, object value, bool noAutoCreate, bool isRoot)
        {
            int key = GetKey(ref type);
...
public static RuntimeTypeModel Create()
{
    return Create(false, ProtoCompatibilitySettingsValue.Default);
        ...
        /// <param name="dest">The destination stream to write to.</param>
        /// <param name="isRoot"></param>
        protected internal abstract void ThrowNestedListsNotSupported(int key, object value, ProtoWriter dest, bool isRoot);
