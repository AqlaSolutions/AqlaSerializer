...
", schema);
        ...
        private static void TestCustomBlobLikeReader(TypeModel model)
        {
            ...
            Assert.AreEqual(20, expected.Length);
            ms.Position = 0;
            ...
            Assert.AreEqual(20, ctx.TotalAllocatedSequence);
            Assert.AreEqual(0, ctx.TotalAllocatedMemory);
            ...
            Assert.AreEqual("0A-14-" + BitConverter.ToString(expected), BitConverter.ToString(ms.GetBuffer(), 0, (int)ms.Length));
        }
        ...
        private static void TestCustomMemoryBlobLikeReader(TypeModel model)
        {
            ...
            Assert.True(blobish.Value.Payload.ToArray().SequenceEqual(expected));
            Assert.AreEqual(0, ctx.TotalAllocatedSequence);
            Assert.AreEqual(20, ctx.TotalAllocatedMemory);
            ...
            // : ISerializer<Blobish>
        {
            ...
            MemoryBlobish ISerializer<MemoryBlobish>.Read(ref ProtoReader state, MemoryBlobish value)
                => state.AppendBytes(value, this);

            void ISerializer<MemoryBlobish>.Write(ref ProtoWriter state, MemoryBlobish value)
            ...
            //Blobish ISerializer<Blobish>.Read(ref ProtoReader state, Blobish value)
            //    => new Blobish(state.AppendBytes(value.Payload, SequenceAllocator));

            //void ISerializer<Blobish>.Write(ref ProtoWriter state, Blobish value)
