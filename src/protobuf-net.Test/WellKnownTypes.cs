...
namespace ProtoBuf.Schemas
{
    ...
    public class WellKnownTypes
    {
        ...
        [Test]
        [TestCase("2017-01-15T01:30:15.01Z")]
        [TestCase("1970-01-15T00:00:00Z")]
        [TestCase("1930-01-15T00:00:00.00001Z")]
...
#pragma warning restore CS0618
            ...
            Assert.AreEqual(seconds, hazTs.Value?.Seconds ?? 0);
            Assert.AreEqual(nanos, hazTs.Value?.Nanos ?? 0);

            // and back again
            hazDt = Serializer.ChangeType<HasTimestamp, HasDateTime>(hazTs);
            Assert.AreEqual(when, hazDt.Value);
        }
        ...
        [Test]
        [TestCase("00:12:13.00032")]
        [TestCase("-00:12:13.00032")]
        [TestCase("00:12:13.10032")]
        [TestCase("00:12:13")]
        [TestCase("-00:12:13")]
        [TestCase("00:00:00.00032")]
        [TestCase("-00:00:00.00032")]
        [TestCase("00:00:00")]
        ...
        private void AssertKnownValue(Timestamp value, DateTime expected)
        {
            ...
            Assert.AreEqual(expected, ChangeType<HasTimestamp, HasDateTime>(runtime, obj).Value);
            Assert.AreEqual(expected, ChangeType<HasTimestamp, HasDateTime>(dynamicMethod, obj).Value);
            Assert.AreEqual(expected, ChangeType<HasTimestamp, HasDateTime>(fullyCompiled, obj).Value);

            var obj2 = new HasDateTime { Value = expected };
            var other = ChangeType<HasDateTime, HasTimestamp>(runtime, obj2).Value ?? new Timestamp();
            Assert.AreEqual(value.Seconds, other.Seconds);
            Assert.AreEqual(value.Nanos, other.Nanos);
        ...
        private void AssertKnownValue(TimeSpan expected, Duration valueToSerialize, Duration valueToDeserialize = null)
        {
            ...
            Assert.AreEqual(expected, ChangeType<HasDuration, HasTimeSpan>(runtime, obj).Value);
            Assert.AreEqual(expected, ChangeType<HasDuration, HasTimeSpan>(dynamicMethod, obj).Value);
            Assert.AreEqual(expected, ChangeType<HasDuration, HasTimeSpan>(fullyCompiled, obj).Value);
            ...
            Assert.AreEqual(valueToDeserialize.Seconds, other.Seconds);
            Assert.AreEqual(valueToDeserialize.Nanos, other.Nanos);
        ...
        public WellKnownTypes() : this() { }


        [Test]
        [TestCase(0, 0, 0, "0A-00")] // nothing
        [TestCase(0.9, 0, 900000000, "0A-06-10-80-D2-93-AD-03")] // field 2 = 900000000
        [TestCase(1.0, 1, 0, "0A-02-08-01")] // field 1 = 1
        [TestCase(1.1, 1, 100000000, "0A-07-08-01-10-80-C2-D7-2F")] // field 1 = 1, field 2 = 100000000 
        [TestCase(-0.9, 0, -900000000, "0A-0B-10-80-AE-EC-D2-FC-FF-FF-FF-FF-01")] // field 2 = -900000000
        [TestCase(-1.0, -1, 0, "0A-0B-08-FF-FF-FF-FF-FF-FF-FF-FF-FF-01")] // field 1 = -1
        [TestCase(-1.1, -1, -100000000, "0A-16-08-FF-FF-FF-FF-FF-FF-FF-FF-FF-01-10-80-BE-A8-D0-FF-FF-FF-FF-FF-01")]
        ...
        public void TestDurationRepresentations(double valueSeconds, long seconds, int nanos, string expectedHex)
        {
            ...
            Assert.AreEqual(seconds, dur.Seconds);
            Assert.AreEqual(nanos, dur.Nanos);
        ...
        [Test] public void Duration_ExpectedValues_1() => AssertKnownValue(TimeSpan.FromSeconds(1), new Duration { Seconds = 1 });
        [Test] public void Duration_ExpectedValues_2() => AssertKnownValue(TimeSpan.FromSeconds(-1), new Duration { Seconds = -1 });
        [Test] public void Duration_ExpectedValues_3() => AssertKnownValue(TimeSpan.FromMilliseconds(1), new Duration { Nanos = 1000000 });
        [Test] public void Duration_ExpectedValues_4() => AssertKnownValue(TimeSpan.FromMilliseconds(-1), new Duration { Nanos = -1000000 });
        [Test] public void Duration_ExpectedValues_5() => AssertKnownValue(TimeSpan.FromTicks(1), new Duration { Nanos = 100 });
        [Test] public void Duration_ExpectedValues_6() => AssertKnownValue(TimeSpan.FromTicks(-1), new Duration { Nanos = -100 });
        [Test] public void Duration_ExpectedValues_7() => AssertKnownValue(TimeSpan.FromTicks(2), new Duration { Nanos = 250 }, new Duration { Nanos = 200 });
        [Test] public void Duration_ExpectedValues_8() => AssertKnownValue(TimeSpan.FromTicks(-2), new Duration { Nanos = -250 }, new Duration { Nanos = -200 });
        ...
        private void TimeSpan_WellKnownEquiv(TypeModel model, TimeSpan time)
        {
            ...
            Assert.AreEqual(seconds, hazD.Value?.Seconds ?? 0);
            Assert.AreEqual(nanos, hazD.Value?.Nanos ?? 0);

            // and back again
            hazTs = ChangeType<HasDuration, HasTimeSpan>(model, hazD);
            Assert.AreEqual(time, hazTs.Value);
        }
