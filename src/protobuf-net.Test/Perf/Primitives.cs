...
#pragma warning restore xUnit1026
        {
            ...
            try
            {
                ...
                var writeState = new ProtoWriter(ms, RuntimeTypeModel.Default);
                try
                {
                ...
                Assert.AreEqual("0A-12-08-C0-8D-C9-B8-C0-B4-B8-E2-3E-10-DE-9C-BF-82-02-18-38", hex);

                ms.Position = 0;
                var readState = new ProtoReader(ms, RuntimeTypeModel.Default);
                try
                {
...
#pragma warning restore xUnit1026
        ...
        private void CheckGuidLayoutImpl(MemoryStream ms, Guid value, string expected)
        {
            ms.Position = 0;
            ms.SetLength(0);
            var state = new ProtoWriter(ms, RuntimeTypeModel.Default);
            try
            {
                state.WriteFieldHeader(1, WireType.String);
                BclHelpers.WriteGuid(state, value);
                state.Close();
            }
            finally
            {
