...
namespace ProtoBuf.Tests
{
    public class BufferWriterTests
    {
        ...
        public void ManualWriter_Stream()
        {
            ...
            var state = new ProtoWriter(ms, RuntimeTypeModel.Default, null);
            try
            {
                ManualWriter(state);
            }
            ...
            Assert.AreEqual("08-2A-12-10-61-62-63-64-65-66-67-68-69-6A-6B-6C-6D-6E-6F-70-1B-1C-22-00", hex);
        }
        ...
        public void ManualWriter_Buffer()
        {
            ...
            var state = new ProtoWriter(bw, RuntimeTypeModel.Default, null);
            try
            {
                ManualWriter(state);
            }
        ...
        class Foo
        {
            ...
            sealed class FooSerializer : ISerializer<Foo>
            {
                ...
                public Foo Read(ref ProtoReader state, Foo value) => value;

                public void Write(ref ProtoWriter state, Foo value) { }
            }
        }

        private void ManualWriter(ref ProtoWriter state)
        {
            try
            {
                state.WriteFieldHeader(1, WireType.Varint);
                Assert.AreEqual(1, state.GetPosition());
                state.WriteInt32(42);
                Assert.AreEqual(2, state.GetPosition());

                state.WriteFieldHeader(2, WireType.String);
                Assert.AreEqual(3, state.GetPosition());
                state.WriteString("abcdefghijklmnop");
                Assert.AreEqual(20, state.GetPosition());

                state.WriteFieldHeader(3, WireType.StartGroup);
                Assert.AreEqual(21, state.GetPosition());
                state.WriteMessage<Foo>(default, null, Foo.Serializer);
                Assert.AreEqual(22, state.GetPosition());

                state.WriteFieldHeader(4, WireType.String);
                Assert.AreEqual(23, state.GetPosition());
                state.WriteMessage<Foo>(default, null, Foo.Serializer);
                Assert.AreEqual(24, state.GetPosition());
        ...
        class ASerializer : ISerializer<A>
        {
            ...
            public ASerializer() => Log = log;
            public ITestOutputHelper Log { get; }
            public A Read(ref ProtoReader state, A value)
            {
                int fieldHeader;
            ...
            public void Write(ref ProtoWriter state, A value)
            {
...
#pragma warning restore CS0618
        ...
        public void WriteWithCustomSerializer_Buffer(int depth)
        {
            ...
            var state = new ProtoWriter(bw.Writer, model, null);
            try
            {
        ...
        public void WriteWithCustomSerializer_Stream(int depth)
        {
            using var ms = new MemoryStream();
            var model = RuntimeTypeModel.Default;
            var state = new ProtoWriter(ms, model, null);
            try
            {
                A obj = CreateModel(depth);
                var ser = new ASerializer(Log);
        ...
        public void RoundTripBasicModel_Buffer(int depth)
        {
            ...
            while (obj != null)
            {
                Assert.AreEqual(depth - level, obj.Level);
                level++;
                obj = obj.Inner;
            }
