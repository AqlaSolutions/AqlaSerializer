...
namespace ProtoBuf.Test
{
    public class NodaTimeTests
    {
        ...
        [Test]
        [TestCase(typeof(NodaTime.Duration), typeof(WellKnownTypes.Duration))]
        [TestCase(typeof(NodaTime.Instant), typeof(WellKnownTypes.Timestamp))]
        public void CanRegisterTypes(Type modelType, Type surrogateType)
        {
            ...
            Assert.AreEqual(surrogateType, metaType.GetSurrogateOrBaseOrSelf(false).Type);
        }
...
", Log(schema));
        ...
        public void CanRoundTripValueWithDuration()
        {
            ...
            static void TestRoundTrip(NodaTimeTests tests, TypeModel model)
            {
                ...
                Assert.AreEqual(obj.Time, clone.Time);
            }
        }
        ...
        public void AssertBytesFromTimeSpanModel()
        {
            ...
            Assert.AreEqual("08-2A-12-0B-08-F4-DE-DD-01-10-C0-ED-86-D7-01-1A-03-61-62-63", Log(BitConverter.ToString(ms.GetBuffer(), 0, (int)ms.Length)));
        }
        ...
        public void AssertBytesFromDateTimeModel()
        {
            ...
            // this is the same output as noted in TestExpectedBinaryOutput
            Assert.AreEqual("08-2A-12-0C-08-80-DC-88-FA-05-10-C0-ED-86-D7-01-1A-03-61-62-63", Log(BitConverter.ToString(ms.GetBuffer(), 0, (int)ms.Length)));
        }
        ...
        public void AllCommonTypesSupported()
        {
            ...
            static void Test(NodaTimeTests test, TypeModel model)
            {
                ...
                Assert.AreEqual("0A-0B-08-F4-DE-DD-01-10-C0-ED-86-D7-01-12-0C-08-80-DC-88-FA-05-10-C0-ED-86-D7-01-1A-07-08-E4-0F-10-08-18-19-22-0A-08-0B-10-0F-18-2B-20-F8-D1-02-28-04", hex);
                ms.Position = 0;
                var clone = model.Deserialize<MakeMeOneWithEverything>(ms);
                Assert.AreNotSame(obj, clone);

                Assert.AreEqual(obj.Duration, clone.Duration);
                Assert.AreEqual(obj.Instant, clone.Instant);
                Assert.AreEqual(obj.LocalDate, clone.LocalDate);
                Assert.AreEqual(obj.LocalTime, clone.LocalTime);
                Assert.AreEqual(obj.IsoDayOfWeek, clone.IsoDayOfWeek);
                Assert.AreEqual(NodaTime.IsoDayOfWeek.Thursday, clone.IsoDayOfWeek);
            }
        }
