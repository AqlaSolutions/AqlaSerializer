...
", schema);
            ...
            Assert.AreEqual(ExpectedHex, GetHex(model, obj));
            var clone = model.DeepClone(obj);
            Assert.AreNotSame(obj, clone);
            Assert.AreEqual(obj.Date, clone.Date);
            Assert.AreEqual(obj.Ip, clone.Ip);
            Assert.AreEqual(obj.ExampleDescription, clone.ExampleDescription);
        }
...
", schema);
        ...
        public sealed class IPAddressSerializer : ISerializer<IPAddress>
        {
            ...
            IPAddress ISerializer<IPAddress>.Read(ref ProtoReader state, IPAddress value)
                => IPAddressParse(state.ReadString());

            void ISerializer<IPAddress>.Write(ref ProtoWriter state, IPAddress value)
