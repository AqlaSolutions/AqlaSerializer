...
@%SystemRoot%\Microsoft.Net\Framework\v2.0.50727\csc.exe /debug- /optimize+ /noconfig /nostdlib /define:FX11 "/out:%OUTDIR%\aqlaserializer.dll" /target:library /unsafe /recurse:protobuf-net\*.cs "/r:%FLIB%\mscorlib.dll" "/r:%FLIB%\system.dll" "/r:%FLIB%\System.IO.dll" /define:MF /define:NO_GENERICS /define:NO_RUNTIME "/r:%FLIB%\Microsoft.SPOT.Native.dll"

@copy build\fx11\CustomerModel.dll "%OUTDIR%"
@copy build\fx11\SampleDTO.dll "%OUTDIR%"

@"%FBIN%\MetaDataProcessor.exe" -loadHints mscorlib "%FLIB%\mscorlib.dll" -parse "%OUTDIR%\aqlaserializer.dll" -minimize -compile "%OUTDIR%\protobuf-net.pe"
